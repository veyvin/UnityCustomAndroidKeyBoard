        ...
        ILayoutElement
    {
        ...
        protected void UpdateLabel()
        {
            if (m_TextComponent != null && m_TextComponent.font != null && !m_PreventFontCallback)
            {
                ...
                string processed;
                if (inputType == InputType.Password|| inputType == InputType.AndroidPassword)
        ...
        private void ActivateInputFieldInternal()
        {
            ...
            if (TouchScreenKeyboard.isSupported)
            {
                ...
                else if (inputType == InputType.AndroidPassword)
                {
                    using (AndroidJavaClass jc = new AndroidJavaClass("com.unity3d.player.UnityPlayer"))
                    {
                        using (AndroidJavaObject jo = jc.GetStatic<AndroidJavaObject>("currentActivity"))
                        {                     
                            jo.Call("OpenKeyBoard");
                        }
                    }
                }
                else
                {
                    m_Keyboard = TouchScreenKeyboard.Open(m_Text, keyboardType, inputType == InputType.AutoCorrect,
                        multiLine);
                }
                   
                //m_Keyboard = (inputType == InputType.AndroidPassword) ?
        ...
        private void EnforceContentType()
        {
            switch (contentType)
            {
                ...
                case ContentType.AndroidPassword:
                {
                    m_LineType = LineType.SingleLine;
                    m_InputType = InputType.AndroidPassword;
